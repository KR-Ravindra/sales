...
def handle_zippy10(excel, wb, unit):

    _SHEET_NAME = "Cycle Time ZIppy 10"

    zippy10_images = {}

    def load_images():
        zippy10_images.update({
            "Zippy10": Image.open("images/zippy/Zippy10.png"),
            "ZippyCycleTime": Image.open("images/zippy/ZippyCycleTime.png"),
        })
    load_images()
    product_name = "Zippy10"
    
    st.image(zippy10_images[product_name], use_column_width=True)
    ...
    with st.form(key='zippy10_form'):
        ...
        throughput_sorts_per_hr = col2.number_input('Throughput (sorts/hour)', min_value=0, max_value=100000, value=7000)
        traffic_factor = col1.slider('Traffic Factor in %', min_value=0, max_value=100, value=15)/100
        charging_factor = col2.slider('Charging Factor in %', min_value=0, max_value=100, value=12)/100
        
        st.image(zippy10_images["ZippyCycleTime"], use_column_width=True)
        col1, col2 = st.columns(2)
        max_barcodes_in_l_sorting_area = col1.slider(f'Max Barcodes in L sorting area ({ convert_to_feet(unit, 49) if unit == "Feet (ft)" else 49  })', min_value=1, max_value=1000, value=70)
        max_barcodes_in_w_sorting_area = col2.slider(f'Max Barcodes in W sorting area ({ convert_to_feet(unit, 31.5) if unit == "Feet (ft)" else 31.5 })', min_value=1, max_value=1000, value=45)
        total_barcodes_in_layout = st.slider('Total Barcodes in Layout', min_value=0, max_value=5000, value=2000)
