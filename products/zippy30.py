...
def handle_zippy30(excel, wb, unit):

    _SHEET_NAME = "Cycle Time ZIppy 30"

    zippy30_images = {}

    def load_images():
        zippy30_images.update({
            "Zippy30": Image.open("images/zippy/Zippy30.png"),
            "ZippyCycleTime": Image.open("images/zippy/ZippyCycleTime.png"),
        })
    load_images()
    product_name = "Zippy30"
    
    st.image(zippy30_images[product_name], use_column_width=True)
    ...
    with st.form(key='zippy30_form'):
        ...
        throughput_sorts_per_hr = col2.number_input('Throughput (sorts/hour)', min_value=0, max_value=100000, value=1000)
        ...
        st.image(zippy30_images["ZippyCycleTime"], use_column_width=True)
        col1, col2 = st.columns(2)
        max_barcodes_in_l_sorting_area = col1.slider(f'Max Barcodes in L sorting area ({ convert_to_feet(unit, 108) if unit == "Feet (ft)" else 108  })', min_value=1, max_value=1000, value=120)
        max_barcodes_in_w_sorting_area = col2.slider(f'Max Barcodes in W sorting area ({ convert_to_feet(unit, 108) if unit == "Feet (ft)" else 108 })', min_value=1, max_value=1000, value=120)
        total_barcodes_in_layout = st.slider('Total Barcodes in Layout', min_value=0, max_value=2000, value=270)
    

        st.markdown("### Single Cycle times")
        col1,col2,col3,col4 = st.columns(4)
        near_combined_weightage = col1.number_input(f'Near % - { convert_to_feet(unit, 10.8) if unit == "Feet (ft)" else 10.8  } ', min_value=0, max_value=100, value=32)/100
        mid_combined_weightage = col2.number_input(f'Mid % - { convert_to_feet(unit, 21.6) if unit == "Feet (ft)" else 21.6 } ', min_value=0, max_value=100, value=32)/100
        far_combined_weightage = col3.number_input(f'Far % - { convert_to_feet(unit, 54) if unit == "Feet (ft)" else 54 } ', min_value=0, max_value=100, value=32)/100
        diagonally_farthest_weightage = col4.number_input(f'Diagonal % - { convert_to_feet(unit, 108) if unit == "Feet (ft)" else 108 } ', min_value=0, max_value=100, value=5)/100
