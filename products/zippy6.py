...
def handle_zippy6(excel, wb, unit):

    _SHEET_NAME = "Cycle Time ZIppy 6"

    zippy6_images = {}

    def load_images():
        zippy6_images.update({
            "Zippy6": Image.open("images/zippy/Zippy6.png"),
            "ZippyCycleTime": Image.open("images/zippy/ZippyCycleTime.png"),
        })
    load_images()
    product_name = "Zippy6"
    
    st.image(zippy6_images[product_name], use_column_width=True)
    ...
    with st.form(key='zippy6_form'):
        
        col1, col2 = st.columns(2)
        distance_bw_induction_and_first_sort = col1.number_input('Distance b/w induction station and first sort point', min_value=0, max_value=100, value=6)
        ...
        st.image(zippy6_images["ZippyCycleTime"], use_column_width=True)
        col1, col2 = st.columns(2)
        max_barcodes_in_l_sorting_area = col1.slider(f'Max Barcodes in L sorting area ({ convert_to_feet(unit, 90) if unit == "Feet (ft)" else 90  })', min_value=1, max_value=1000, value=150)
        max_barcodes_in_w_sorting_area = col2.slider(f'Max Barcodes in W sorting area ({ convert_to_feet(unit, 120) if unit == "Feet (ft)" else 120 })', min_value=1, max_value=1000, value=200)
